...
Para definir contrastes, primero debemos convertir las variables `Psychotherapy` y `Drug` en factores.
...
modeloC <- lm(Mood ~ 1 + Psychotherapy + Drug, data = experiment)
modeloA <- lm(Mood ~ 1 + Psychotherapy + Drug + Psychotherapy*Drug, data = experiment)
```
...
Recordemos que si un factor tiene $m$ niveles, solo podemos crear $m-1$ niveles.
...
`(Intercept)`: Media general o gran media de `Mood`, es decir, media de `Mood` sin tomar en cuenta el grupo.
